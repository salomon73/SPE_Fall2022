classdef espic2dhdf5
    ...
    properties
        ...
        species_moments % containing moments for species (parts)
    ...
    methods
            ...
            if(readparts)
                ...
                if(obj.nbspecies >1)
                    ...
                    for i=2:obj.nbspecies
                        ...
                        obj.species_moments(i-1).N=splinedensity(obj.fullpath, sprintf('/data/part/%2d/moments',i), obj.knotsr, obj.knotsz, obj.femorder, obj.CellVol, 1, geomweight, 1);
                        obj.species_moments(i-1).fluidUR=splinevelocity(obj.fullpath, sprintf('/data/part/%2d/moments',i), obj.knotsr, obj.knotsz, obj.femorder, obj.vnorm, geomweight, 2);
                        obj.species_moments(i-1).fluidUTHET=splinevelocity(obj.fullpath, sprintf('/data/part/%2d/moments',i), obj.knotsr, obj.knotsz, obj.femorder, obj.vnorm, geomweight, 3);
                        obj.species_moments(i-1).fluidUZ=splinevelocity(obj.fullpath, sprintf('/data/part/%2d/moments',i), obj.knotsr, obj.knotsz, obj.femorder, obj.vnorm, geomweight, 4);
                    end
                end
            end
        ...
        function Gamma=Metallicflux(obj,timestep,subdiv)
            ...
            N=obj.N(:,:,timestep(1));
            n=griddedInterpolant(z,r,N'); % choose as function of species # (dep espece)
            ...
            for j=1:length(timestep)
                n.Values=obj.N(:,:,timestep(j))';   % dependence esp
        ...
        function gamma_species = MetallicFlux_species(obj,timestep,subdiv)
         % Computes the particle flux at time obj.t2d(timestep) on the
            % metallic boundaries for species 2, supposedly the ions
            ...
            [z,r]=ndgrid(obj.zgrid,obj.rgrid);
            N=obj.species_moments.N;
            N = N(:,:,timestep(1));
            n=griddedInterpolant(z,r,N'); % choose as function of species # (dep espece)
            uz=griddedInterpolant(z,r,obj.species_moments.fluidUZ(:,:,timestep(1))');
            ur=griddedInterpolant(z,r,obj.species_moments.fluidUR(:,:,timestep(1))');
            % we get the density and fluid velocities at the desired time
            % steps and interpolate them at the boundary position
            for j=1:length(timestep)
                n.Values=obj.species_moments.N(:,:,timestep(j))';   % dependence esp
                uz.Values=obj.species_moments.fluidUZ(:,:,timestep(j))';
                ur.Values=obj.species_moments.fluidUR(:,:,timestep(j))';
                for i=1:length(p)
                    bp=p{i};
                    gamma{i}(:,j)=n(bp(1:2,:)').*(ur(bp(1:2,:)').*Nr{i}'+uz(bp(1:2,:)').*Nz{i}');
                end
...
%         
        function [I, pos]=OutCurrents(obj,timestep, subdiv)
            ...
            qe = obj.qsim/obj.weight; % electrons charge
            flux=obj.Axialflux(timestep,[1 obj.nz+1]);
            Iz=squeeze(trapz(obj.rgrid,flux.*obj.rgrid)*2*pi*qe);
            Iz(1,:)=-Iz(1,:);
        ...
        function out=displaycharge(obj,scalet,f,linelegend)
            ...
            try
                plot(ax,obj.t0d(tmin:tmax)/taucol,abs(obj.npart(tmin:tmax)*obj.qsim),'linewidth',2,'displayname',linelegend)
                hold on
            ...
            out.time   = obj.t0d(tmin:tmax)/taucol;
            out.charge = abs(obj.npart(tmin:tmax)*obj.qsim);
        end
        ...
        function out = displaytotcurrevol(obj,timesteps,species_id,all_cur_id, toptitle,scalet,dens,subdiv,nmean)
            ...
            % hack to sum ionic + electronic current
            if all_cur_id==1
                [el_curr,pos]=obj.OutCurrents(timesteps,subdiv);
                P=obj.neutcol.neutdens*obj.kb*300/100;% pressure at room temperature in mbar
                out.elec_currents=el_curr/P;
                
                [ion_curr,pos]=obj.OutCurrents_species(timesteps,subdiv);
                P=obj.neutcol.neutdens*obj.kb*300/100;% pressure at room temperature in mbar
...
% 
% function displaytotcurrevol(obj,timesteps,species_id,all_cur_id,toptitle,scalet,dens,subdiv,nmean)
%             % Computes and display the time evolution of the outgoing
%             % currents at time obj.t2d(timesteps)
%             %scalet=true scales the time by the ellastic collision
%             %frequency
...
%                 end
%             else 
%                 [currents_el,pos]=obj.OutCurrents(timesteps,subdiv);
%                 P=obj.neutcol.neutdens*obj.kb*300/100;% pressure at room temperature in mbar
%                 currents_el=currents_el/P;
%                 [currents_ion,pos]=obj.OutCurrents_species(timesteps,subdiv);
%                 currents_ion = current_ion/P;
%             end
%             
%             f=figure('Name',sprintf('%s Charges',obj.name));
%             tiledlayout(2,1)
...
%         end
        ...
        function display_ionicconfigurration(obj, fieldstart, fieldend, species_id)
           %displayconfiguration plot the configuration of the simulation
            ...
            % species_id: gathers the ion species indices in an array.
            for ii = 1 :species_id(end-1) 
                dens(ii,:,:)=mean(obj.species_moments(ii).N(:,:,fieldstart:fieldend),3);
            end
            geomw=obj.geomweight(:,:,1);
            maxdens=max(dens(:));
            geomw(geomw<0)=0;
            ...
            %% electron density
            for ii =1:2
                h(ii)=contourf(ax1,obj.zgrid*1000,obj.rgrid*1000,dens(ii,:,:),50,'Displayname','n_e [m^{-3}]', 'linestyle','none');
                hold on;
            end
            colormap(flipud(hot));
            %% Magnetic field lines
        ...
        function displaySurfFlux(obj,timestep, subdiv, species_id,scalet)
            ...
            if scalet == false
                ...
                title(sprintf('t/tau_d=%4.2f [-]', time/taucol))
            end
