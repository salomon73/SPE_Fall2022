...
i=1;
points=linspace(0,1,npts)'*([zmin,rc]-[zc rmin])+[zc rmin];
geomcells{1}.Z(i+(1:npts-1))=points(2:end,1);
geomcells{1}.R(i+(1:npts-1))=points(2:end,2);

i=i+npts-1;
points=linspace(0,1,npts)'*([zc,rmax]-[zmin rc])+[zmin rc];
geomcells{1}.Z(i+(1:npts-1))=points(2:end,1);
geomcells{1}.R(i+(1:npts-1))=points(2:end,2);

i=i+npts-1;
points=linspace(0,1,npts)'*([zmax,rc]-[zc rmax])+[zc rmax];
geomcells{1}.Z(i+(1:npts-1))=points(2:end,1);
geomcells{1}.R(i+(1:npts-1))=points(2:end,2);

i=i+npts-1;
points=linspace(0,1,npts)'*([zc,rmin]-[zmax rc])+[zmax rc];
geomcells{1}.Z(i+(1:npts-1))=points(2:end,1);
geomcells{1}.R(i+(1:npts-1))=points(2:end,2);
...
overwrite=true;
if save
    savegeomtoh5('test_rot_square_in.h5',geomcells,1e-2,overwrite);
end
