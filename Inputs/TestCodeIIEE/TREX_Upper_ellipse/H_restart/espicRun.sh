...
res_folder='/scratch/sguincha/TestCodeIIEE/TREX_Fields/Results/'
mkdir -p $res_folder
cat > job.in << EOM 
...
&BASIC
  ...
  nbcelldiag=1,
  itcelldiag=20,
  resfile='${res_folder}TestTrex_Fields.h5'
  rstfile='${res_folder}TestIIEE_restart.h5'
  nbspecies=2,
  partfile='ion_tracers.in','electrons.in',
  nbaddtestspecies = 2,
  addedtestspecfile = 'ion_tracers.in','electrons.in',
  ...
  n0=-3e17, dt=7E-12                    !Initial plasma density in plasmadim
...
mpirun -np 1  /home/sguincha/espic2d/src/espic2d job.in
